...
namespace Ginnay.ProxySpider
{
	public class HtmlHelper
	{
		...
		public static bool GetHtml(Stream s, string contentEncoding, out string html)
		{
			...
			try
			{
				while ((c = s.Read(buffer, 0, BUFFER_SIZE)) > 0)
				{
					total += c;
					ms.Write(buffer, 0, c);
				}
			...
			return GetHtml(ms.ToArray(), contentEncoding, false, false, out html);
		}
